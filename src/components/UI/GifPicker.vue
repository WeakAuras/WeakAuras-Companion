<template>
  <div class="gifpicker">
    <header>
      <div>
        <span :class="{ invisible: search === '' }" class="material-icons arrow_back" @click="search = ''">arrow_back</span>
        <div>
      ...
      <div>
        <div>
          <div v-if="search !== '' && results && results.length" class="grid" v-scroll="loadMore">
            <div v-for="(result, r) in results" :key="r" class="grid_images"
              :style="`background-image: url(${result.media[0].tinygif.url})`" @click="send(result)" />
          </div>
          <div v-else-if="tags && tags.length" class="grid">
            <div v-for="(tag, t) in tags" :key="t" class="grid_images" :style="`background-image: url(${tag.image})`"
