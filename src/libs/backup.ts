...
function backupIfRequired(filename, config, previousSize, accountName, callback, addonName) {
  if (config?.active && filename) {
    ...
    if (stats.size !== previousSize) {
      ...
      archive.append("If you want to restore this backup, close WOW first, then move the WeakAuras.lua file into your saved variable folder (World of Warcraft\\_retail_\\WTF\\Account\\ACCOUNTNAME\\SavedVariables).", { name: "README.txt" });
      archive.finalize();
    }
  }
}
